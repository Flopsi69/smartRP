...
$(document).ready(function () {
  ...
  $('.rating-tabs__item').on('click', function (e) {
    ...
    $('.rating-tab[data-tab="' + $(this).data('tab-target') + '"]')
      .slideToggle('slow')
      .addClass('active');
  });
...
function Circlebar(prefs) {
  ...
  this.initialise = function () {
    ...
    that.maxValue =
      parseInt(attribs.circleMaxvalue) || parseInt(prefs.maxValue) || 60;
    that.counter =
...
if ($('#circle-1').length) {
  let currentLevel = parseInt($('#circle-1').data('current'));
  new Circlebar({
    element: '#circle-1',
    type: 'progress',
...
if ($('#circle-2').length) {
  let currentExp = parseInt($('#circle-2').data('current'));
  new Circlebar({
    element: '#circle-2',
    type: 'progress',
    maxValue: currentExp,
...
function donateFunction() {
  ...
  if (!!donateInterForm) {
    donateInterForm.addEventListener('submit', function (event) {
      event.preventDefault();
      if (validate(this)) {
        donateInterForm.querySelector("input[name='ik_pm_no']").value =
          donateInterForm.querySelector('#name').value;
        donateInterForm.querySelector("input[name='ik_am']").value =
          donateInterForm.querySelector('#money').value;
